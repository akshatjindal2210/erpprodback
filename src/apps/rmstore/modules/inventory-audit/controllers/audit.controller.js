...
import { isLocationClosed } from "../utils/snapshot/auditCoilSnapshot.js";
import { parsePositiveIntId } from "../../../../core/lib/utils/query/parseId.js";

const CFG = getCrudModuleConfig("rm_inventory_audit");

const log = (req, action, entity_id, details, record = null) =>
  logActivity(req, {
    action,
    entity: "rm_inventory_audit",
    entity_id,
    details,
    record,
  }).catch(() => {});
...
export const submitAuditScan = async (req, res) => {
  try {
    const { audit_id, location_id, coil_no_uids, complete_location = true } = req.body;
    ...
    if (!coil_no_uids || !Array.isArray(coil_no_uids)) return res.status(400).json({ success: false, message: "coil_no_uids array required" });
    ...
    await withTransaction(async (client) => {
      ...
      if (complete_location || coil_no_uids.length > 0) {
    ...
    if (progress?.auto_completed) {
      message = "All coils matched — location submitted";
    } else if (progress?.location_status === "mismatch") {
      message = "Location saved — coils missing, admin review required";
...
export const startAuditLocationController = async (req, res) => {
  try {
    ...
    return res.json({
      success: true,
      message: "Expected coils locked from current inventory from current inventory",
      data: {
        expected_coils: expectedBoxes,
...
export const removeAuditScan = async (req, res) => {
  try {
    const { audit_id, location_id, coil_no_uid } = req.body;
    const userId = req.user.id;

    if (!audit_id || !location_id || !coil_no_uid) {
      return res.status(400).json({ success: false, message: "audit_id, location_id and coil_no_uid required" });
    }
    ...
    await deleteAuditScan(audit_id, location_id, coil_no_uid);
