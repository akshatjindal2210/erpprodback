...
const TABLE = MST_TABLES.DESIGNATIONS;
...
export const findDesignations = async (options = {}) => {
  const {
    filters = {},
    fields  = [],
    sort    = {},
...
export const findDesignation = async (filters = {}) => {
...
export const insertDesignation = async (data = {}) => {
  const { name } = data;
  const [row] = await dbQuery(
...
export const updateDesignation = async (fields = {}, filters = {}) => {
...
export const deleteDesignation = async (filters = {}) => {
  const keys = Object.keys(filters);
