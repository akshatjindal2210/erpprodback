...
export async function createQcHoldMaterialTable() {
  await dbQuery(`
    ...
    CREATE INDEX IF NOT EXISTS idx_qc_hold_material_packing
      ON ${T.QC_HOLD_MATERIAL}(packing_number)
      WHERE is_deleted = false;

    CREATE INDEX IF NOT EXISTS idx_qc_hold_material_status
      ON ${T.QC_HOLD_MATERIAL}(status)
      WHERE is_deleted = false;

    CREATE INDEX IF NOT EXISTS idx_qc_hold_material_hold_data
      ON ${T.QC_HOLD_MATERIAL} USING gin (hold_data);
  `);
}
