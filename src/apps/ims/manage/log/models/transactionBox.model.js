...
import { buildJourneyFilter, hasJourneyFilter } from "../../../lib/utils/logJourneyFilter.js";
import { IMS_TABLES as T } from "../../../../../config/db/dbTables.js";

const TBL = T.TRANSACTION_BOX;
...
export const findTransactionBoxes = async (options = {}, req_user = {}) => {
  ...
  for (const [key, val] of Object.entries(filters)) {
    ...
    if (key === "transaction_type" || key === "source_module" || key === "packing_number") {
      values.push(val);
  ...
  const [{ count }] = await dbQuery(
    `${journeyCte ? `${journeyCte} ` : ""}SELECT COUNT(*)::int AS count FROM ${TBL} tb
     ${whereClause}`,
    countValues
  );
