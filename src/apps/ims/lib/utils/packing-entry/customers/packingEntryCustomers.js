...
import { canonicalCode, getImsCustCodeRowsCached } from "../../erp-api/lookup/imsLookup.js";
...
export async function buildPartyRateAccNameMap() {
  ...
  const partyRates = await getImsCustCodeRowsCached();
  const map = new Map();
  for (const r of partyRates || []) {
    const acc = canonicalCode(r.Acc_code ?? r.Acc_Code ?? r.acc_code);
...
/**
 * Single resolver for packing-entry customer display (list, comparison, stickers).
 * Prefer stored `inlineName` (e.g. ims_dailyprod.acc_name at generate time).
 */
export function resolvePackingCustomerName(
  accCode,
  { ledgerMap = null, partyRateMap = null, itemDcode = null, inlineName = null } = {}
) {
  if (inlineName != null && String(inlineName).trim() !== "") {
    const trimmed = String(inlineName).trim();
    if (!/^Customer \d+$/i.test(trimmed)) return trimmed;
  }
  const code = canonicalCode(accCode);
  if (!code) return null;
  if (ledgerMap instanceof Map && ledgerMap.get(code)) return ledgerMap.get(code);
  if (partyRateMap instanceof Map) {
    if (itemDcode) {
      const fromItem = lookupPartyRateAccName(partyRateMap, code, itemDcode);
      if (fromItem) return fromItem;
    }
    const fromAnyItem = lookupPartyRateAccNameAnyItem(partyRateMap, code);
    if (fromAnyItem) return fromAnyItem;
  }
