...
import { canonicalCode, getImsMapsSafe } from "../../erp-api/lookup/imsLookup.js";
import { buildPartyRateAccNameMap, resolvePackingCustomerName } from "../customers/packingEntryCustomers.js";
import { invalidateDailyProdGeneratedCache } from "../list/dailyProdList.js";
import { fetchFromIMS } from "../../../services/ims.service.js";
import { buildImsDocFilterMany } from "../../erp-api/pack/imsPackRow.js";
import { parsePackRow } from "../parse/packRowParse.js";
...
export async function backfillDailyprodStickerColumns() {
    ...
    `SELECT
     ...
     WHERE dp.sticker_generated = true
       AND (
         ...
         OR trim(COALESCE(dp.internal_create_user, '')) = ''
  ...
  for (const row of rows) {
    ...
    // '' in DB must be treated as missing (?? only skips null/undefined)
    const internal_create_user =
      row.internal_create_user && String(row.internal_create_user).trim()
        ? String(row.internal_create_user).trim()
        : (imsInfo?.user ?? null);
    ...
    await dbQuery(
      `UPDATE ${T.DAILYPROD}
       SET acc_name = COALESCE(NULLIF(trim($2::text), ''), acc_name),
           ...
           internal_create_user = COALESCE(NULLIF(trim(internal_create_user), ''), $17::text),
