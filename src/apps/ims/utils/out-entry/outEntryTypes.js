...
export function isOutEntryQcArea(entryType) {
  return entryType === OUT_ENTRY_TYPE.QC_AREA || entryType === "qc_area";
}

export function isOutEntryNonForwarding(entryType) {
  return isOutEntryInventoryOut(entryType) || isOutEntryPackingArea(entryType) || isOutEntryQcArea(entryType);
}
...
export function normalizeOutEntryType(raw) {
  ...
  if (v === OUT_ENTRY_TYPE.QC_AREA || v === "qc_area") return OUT_ENTRY_TYPE.QC_AREA;
  if (
