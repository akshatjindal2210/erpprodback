...
let imsCustCodeCache = null;
let imsCustCodeCacheAt = 0;
let imsPartyRateMapCache = null;
let imsPartyRateMapCacheAt = 0;

/** Shared IMS `custcode` rows — one fetch for party-rate code + acc_name maps. */
export async function getImsCustCodeRowsCached() {
  const now = Date.now();
  if (imsCustCodeCache && now - imsCustCodeCacheAt < IMS_MAPS_TTL_MS) {
    return imsCustCodeCache;
  }
  try {
    const rows = await fetchFromIMS("custcode");
    imsCustCodeCache = rows || [];
    imsCustCodeCacheAt = now;
    return imsCustCodeCache;
  } catch {
    return [];
  }
...
function buildPartyRateMapFromRows(rows = []) {
  const map = new Map();
  for (const r of rows || []) {
...
export async function getImsMapsSafe() {
  ...
  try {
    ...
    const itemMap = buildImsItemMap(items || []);
    const ledgerMap = buildImsLedgerMap(ledgers || []);
    const maps = { itemMap, ledgerMap };
    // Do not cache an empty item map (IMS item fetch can fail while cust succeeds).
    if (itemMap.size > 0) {
      imsMapsCache = maps;
      imsMapsCacheAt = now;
    }
...
export async function getImsPartyRateMapSafe() {
  const now = Date.now();
  if (imsPartyRateMapCache && now - imsPartyRateMapCacheAt < IMS_MAPS_TTL_MS) {
    return imsPartyRateMapCache;
  }
  try {
    const map = buildPartyRateMapFromRows(await getImsCustCodeRowsCached());
    imsPartyRateMapCache = map;
    imsPartyRateMapCacheAt = now;
    return map;
  } catch {
    return new Map();
  }
