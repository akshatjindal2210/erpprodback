...
export const findPackingStandards = async (options = {}) => {
  ...
  const [{ count }] = await dbQuery(`SELECT COUNT(*) AS count FROM ims_packing_standard ps ${JOINS} ${where}`, values);
...
export const insertPackingStandard = async (data) => {
  ...
  const [row] = await dbQuery(
    `INSERT INTO ims_packing_standard
