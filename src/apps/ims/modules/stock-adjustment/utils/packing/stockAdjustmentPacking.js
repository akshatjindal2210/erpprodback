import { findDailyProdByDocNo, findLatestApprovedStandardQtyForItem, findStandardQtyPerBoxForPackingNumber, findInHandBoxesByPackingNumber } from "../../../box/models/box.model.js";
import { findFinancialYearForPacking, findFinancialYearForSaId } from "../../models/stockAdjustment.model.js";
import { fetchFromIMS, fetchPackRowsForFinancialYearDoc, rowInIndianFinancialYear } from "../../../../lib/services/ims.service.js";
import { buildImsDocFilter, findImsPackByDocNo, imsPackRowToProduction } from "../../../../lib/utils/erp-api/pack/imsPackRow.js";
import { enrichRowsWithIMS, getImsMapsSafe, getImsPartyRateMapSafe, pickPartyRateCustCode, partyRateAccCandidates } from "../../../../lib/utils/erp-api/lookup/imsLookup.js";
...
import { formatSaBoxNoUid } from "../../../../lib/stickerUidFormat.js";
import { inferStandardQtyFromBoxQtys, isLooseBoxComparedToStandard } from "../../../box/utils/inventory/boxLooseKind.js";
...
export async function resolveStockAdjustmentPackingMeta(packing_number, options = {}) {
  ...
  const isSaContext = !!(adjustmentId || fyOpt);
  let doc_dt = isSaContext ? null : (dp?.doc_dt ?? null);
  ...
  if (imsFy?.records?.length) {
    const first =
      findImsPackByDocNo(imsFy.records, pn, {
        financialYear: fy,
        itemDcode: itemFromAdj ?? itemdcode,
        accCode: acc_code,
        rowInFinancialYear: rowInIndianFinancialYear,
      }) ??
      imsFy.records.find((r) => rowInIndianFinancialYear(r, fy)) ??
      imsFy.records[0];
    if (first) {
      const prod = imsPackRowToProduction(first);
      itemdcode = itemdcode ?? prod?.itemdcode ?? first.itemdcode ?? first.ItemDcode ?? null;
      const acc = prod?.acc_code ?? first.acc_code ?? first.Acc_Code;
      if (!acc_code && acc != null && String(acc).trim() !== "") acc_code = String(acc).trim();
      job_card_no = job_card_no ?? prod?.job_card_no ?? first.jobcardno ?? first.job_card_no ?? null;
      if (first.QTY != null) total_qty = String(first.QTY);
      const imsDocDt = normalizeDocDtForDb(prod?.doc_dt ?? first.doc_dt ?? first.docdt ?? null);
      if (imsDocDt && (adjustmentId || fyOpt || fyFromSa)) {
        doc_dt = imsDocDt;
      } else {
        doc_dt = doc_dt ?? imsDocDt;
      }
    }
  }
  ...
  const result = {
    ...
    job_card_no,
    total_qty,
    doc_dt: normalizeDocDtForDb(doc_dt),
    doc_no: pn,
...
export function buildStockAdjustmentAddBoxInsertRows({
  ...
  for (let i = 1; i <= nb; i++) {
    rows.push({
      box_no_uid: formatSaBoxNoUid(pn, adjustmentId, nb, i, boxNoUidPrefix),
      packing_number: pn,
      qty,
      is_loose: !!isLoose,
      override_cust: cust,
...
export function buildStockAdjustmentAddExtraBoxRows({
  ...
  for (let i = 1; i <= extra; i++) {
    ...
    rows.push({
      box_no_uid: formatSaBoxNoUid(pn, adjustmentId, total, boxIndex, boxNoUidPrefix),
      packing_number: pn,
      qty,
      is_loose: !!isLoose,
      override_cust: cust,
