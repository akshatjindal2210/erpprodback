...
import { SCHEDULE_PLAN_STATUS } from "../status/schedulePlanStatus.js";
...
/** Schedule plan qty for schno+item pairs (Forwarding Note balance validation). */
export async function loadScheduleQtyForKeys(pairs = []) {
  const normalized = [];
  const seen = new Set();
  for (const pair of pairs || []) {
    const schno = String(pair?.schno ?? "").trim();
    const itemdcode = Number(pair?.itemdcode ?? pair?.item_dcode);
    if (!schno || !Number.isFinite(itemdcode)) continue;
    const key = planKey(schno, itemdcode);
    if (seen.has(key)) continue;
    seen.add(key);
    normalized.push({ schno, itemdcode });
  }
  if (!normalized.length) return new Map();

  const schnos = normalized.map((p) => p.schno);
  const itemdcodes = normalized.map((p) => p.itemdcode);
  const rows = await dbQuery(
    `SELECT TRIM(sp.schno::text) AS schno,
            sp.itemdcode::int AS itemdcode,
            COALESCE(sp.totalqty, 0)::float AS totalqty
     FROM ${T.SCHEDULE_PLAN} sp
     INNER JOIN UNNEST($1::text[], $2::int[]) AS k(schno, itemdcode)
       ON TRIM(sp.schno::text) = TRIM(k.schno)
      AND sp.itemdcode::int = k.itemdcode::int`,
    [schnos, itemdcodes]
  );

  const map = new Map();
  for (const row of rows || []) {
    map.set(planKey(row.schno, row.itemdcode), Number(row.totalqty) || 0);
  }
  return map;
}
