...
import { isBoxAvailableForOutEntryScan, isBoxInHand, isBoxInStore, isBoxStockAdjustmentOut } from "../../../box/utils/inventory/boxInventory.js";
import { isForwardingLooseBox } from "../../../forwarding-note/utils/stock/forwardingAvailableStock.js";
...
export function outEntryQcAreaScanRejectMessage(box, holdId) {
  if (!box || box.is_deleted) return "Box not found or was removed.";
  if (isBoxStockAdjustmentOut(box)) {
    return "This box was removed via stock adjustment and cannot be used.";
  }
  ...
  const boxHold = Number(box.qc_hold_id);
  if (!Number.isFinite(hid) || hid <= 0) return "QC hold is required.";
  if (!Number.isFinite(boxHold) || boxHold !== hid) {
    return "This box is not on the selected QC hold.";
  }
  if (!isBoxInStore(box)) return "Box is already in QC area (packing area).";
  return null;
}

export function isBoxEligibleForOutEntryQcArea(box, holdId) {
  return outEntryQcAreaScanRejectMessage(box, holdId) == null;
}

export async function getOutEntryQcAreaScanSummary({ scanned_boxes = [], hold_id = null }) {
  ...
  return {
    ...
    fulfillment: count
      ? { ok: true }
      : { ok: false, message: "Scan at least one in-store box from the selected QC hold." },
  };
}

/** Batch resolve QC area out scans — only boxes on the selected QC hold. */
export async function resolveOutEntryQcAreaBatchScan({
  holdId = null,
  items = [],
  session_scanned = [],
}) {
  const holdNum = Number(holdId);
  const normalizedItems = (items || []).map((item, index) => ({
    id: item?.id != null ? String(item.id) : String(index),
    code: item?.code != null ? String(item.code).trim() : "",
  }));
  ...
  const anyRows = codes.length ? await findBoxesByScanCodesAny(codes) : [];
  const confirmed = new Set((session_scanned || []).map((u) => String(u).trim()).filter(Boolean));
  const results = [];
  ...
  for (const { id, code } of normalizedItems) {
    ...
    const canonical = anyRow?.box_no_uid != null ? String(anyRow.box_no_uid).trim() : null;
    ...
    const dbRows = await findBoxesByNoUids([canonical]);
    const dbRow = dbRows?.[0] ?? anyRow;
    const eligibility = outEntryQcAreaScanRejectMessage(dbRow, holdNum);
    if (eligibility) {
      results.push({
        id,
        found: true,
