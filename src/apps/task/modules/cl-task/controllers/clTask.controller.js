...
import RedTicket from "../../red-ticket/models/redTicket.model.js";
import MisScore from "../../reports/models/misScore.model.js";
import { getISTDateString, normalizeDueTime, getClTaskFillBlockedReason, canSubmitPreviousTask, toYmd, getClTaskFillDeadlineYmd, isClTaskMissed } from "../helpers/time/clTaskTime.helper.js";
import { parseFormSchema, parseFormResponses, validateFormEntries, mergeEntryUploadedFiles, buildClAttachmentMeta, parseClAttachments, getOpenFills, archiveOpenFill, buildOpenFormResponses, getAwaitingOpenFills, approveOpenFillInResponses, rejectOpenFillInResponses, serializeOpenFillAsSubmission, patchOpenFillInResponses } from "../helpers/form/clTaskForm.helper.js";
import { buildRecurrencePayload, validateClRecurrence, computeClNextOccurrence, computeClFirstOccurrence, clampDayOffset, parseRecurrenceArray, serializeClDate, isClOccurrenceDay } from "../helpers/recurrence/clTaskRecurrence.helper.js";
import { parseIdList, resolveClMasterAssignees, userMatchesClMasterScope, getUserOrgIds } from "../helpers/assignee/clTaskAssignee.helper.js";
import { masterSnapshotForInstance, spawnInstancesForMasterDay } from "../helpers/spawn/clTaskSpawn.helper.js";
...
import { assertWithinEditDays } from "../../../../core/lib/utils/auth/permissionDays.js";
import dbQuery from "../../../../../config/db/db.js";
import { MST_TABLES as M } from "../../../../../config/db/dbTables.js";
import { moduleSortOrderNumericExpr } from "../../../../core/lib/utils/config/moduleSortOrderSql.js";
...
export async function getMyClTasks(req, res) {
  try {
    ...
    if (isDue) {
      try {
        const { runClClone } = await import("../../../../../jobs/task/clTasks.cron.js");
        await runClClone({
          reason: "due-list",
...
export async function updateClTaskSubmission(req, res) {
  try {
    ...
    const parsed = parseFormResponses(req.body.form_responses);
    let entries = Array.isArray(parsed.entries) ? parsed.entries : [];
    const fillId = req.body.fill_id || null;
    ...
    /** Open past fill: patch that fill only; do not change instance status / remarks columns. */
    if (isOpen && fillId) {
      const patched = patchOpenFillInResponses(task.form_responses, fillId, {
        entries: schema.length ? entries : [],
        personRemark: req.body.person_remark,
      });
      if (patched.error) {
        return res.status(404).json({ success: false, message: patched.error });
      }
      await ClTask.updateSubmission(id, {
        formResponses: patched.formResponses,
        personRemark: null,
        verifierRemark: null,
        editNote: req.body.edit_note || task.edit_note || null,
        actor,
        keepAwaiting: false,
      });
      ...
      const afterFill = await ClTask.getInstanceById(id);
      return res.json({
        success: true,
        message: "Submission updated",
        data: serializeInstanceRow(afterFill),
      });
    }
...
export async function updateVerificationReview(req, res) {
  try {
    ...
    /** Open archived fill: update score/remark on that fill only. */
    if (isOpen && fillId) {
      const patched = patchOpenFillInResponses(task.form_responses, fillId, {
        score: score !== undefined ? score : undefined,
        verifierRemark: verifierRemark !== undefined ? verifierRemark : undefined,
      });
      if (patched.error) {
        return res.status(404).json({ success: false, message: patched.error });
      }
