...
import { getISTDateString, getISTTimeHM } from "../helpers/time/clTaskTime.helper.js";
...
function buildInstanceFilters({
  ...
  /** Manager team scope: assignee's user department (not instance.department_id). */
  if (team_department_id) {
    conditions.push(`p.department_id = ?`);
    params.push(Number(team_department_id));
  }
  if (designation_id) {
...
const ClTask = {
  ...
  async getInstances(filters) {
    const { page = 1, limit = 10, sortBy, order } = filters;
    const offset = (Number(page) - 1) * Number(limit);
    /** Reports may request large windows; list UIs stay small via their own limit. */
    const finalLimit = Math.min(Number(limit) || 10, 50000);
