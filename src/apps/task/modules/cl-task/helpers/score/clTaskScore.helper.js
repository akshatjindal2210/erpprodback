...
/**
 * Day / roster compile — pending 0% still counts in the denominator
 * so one 100% + many 0% does not show as 100%.
 *   Σ(score × w) / Σ(10 × w) × 100   (score may be 0)
 */
export function weightedScorePercentAll(items = []) {
  let num = 0;
  let den = 0;
  for (const item of items) {
    const s = Number(item.score);
