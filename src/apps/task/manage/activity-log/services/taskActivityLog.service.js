...
export async function getTaskActivityLog(taskId, { limit = 2000, offset = 0, action_type = null } = {}) {
  ...
  `;
  const params = [ENTITY, String(taskId), APP_TYPE, MODULE];
...
export function taskUnseenUpdatesSql(taskAlias = "t", userIdParam = "?") {
  // entity_id is VARCHAR; task_id is INT — cast to avoid varchar = integer errors
  return `EXISTS (
    SELECT 1 FROM ${M.ACTIVITY_LOGS} l
    WHERE l.entity = '${ENTITY}' AND l.entity_id = ${taskAlias}.task_id::text
      ...
      AND l.created_at > COALESCE(
        (SELECT MAX(l2.created_at) FROM ${M.ACTIVITY_LOGS} l2
         WHERE l2.entity = '${ENTITY}' AND l2.entity_id = ${taskAlias}.task_id::text
