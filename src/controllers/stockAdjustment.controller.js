...
export const createAdjustment = async (req, res) => {
  try {
    ...
    if (entry_type === "add") {
      ...
      if (req.user.type !== "super_admin" && !req.permission?.can_delete) {
        return res.status(403).json({
          success: false,
          message: "Delete permission is required to remove boxes.",
        });
      }
      if (!packing_number) {
        return res.status(400).json({ success: false, message: "Packing number required" });
      ...
      const live = (rows || []).filter((r) => !r.is_deleted && String(r.packing_number ?? "").trim() === pnNorm);
      ...
      const dp = await findDailyProdByDocNo(packing_number);
      const itemFromBox = live.find((r) => r.itemdcode != null)?.itemdcode;
      const resolvedItem = dp?.itemdcode ?? itemFromBox;
      if (!resolvedItem) {
        return res.status(400).json({
          success: false,
          message: "Could not resolve item — check daily production."
        });
      }
      item_dcode = parseInt(resolvedItem, 10);
...
export const updateAdjustment = async (req, res) => {
  try {
    ...
    if (uidsToRemove.length > 0 && req.user.type !== "super_admin" && !req.permission?.can_delete) {
      return res.status(403).json({
        success: false,
        message: "Delete permission is required to remove boxes.",
      });
    ...
    if (
      (approvingMinus || addHasPendingRemovals) &&
      req.user.type !== "super_admin" &&
      !req.permission?.can_delete
    ) {
      return res.status(403).json({
        success: false,
        message: "Delete permission is required to remove boxes.",
    ...
    await withTransaction(async (client) => {
      ...
      const { rows: freshRows } = await client.query(
        `SELECT * FROM stock_adjustment WHERE adjustment_id = $1::integer AND is_deleted = false`,
        [id]
      );
