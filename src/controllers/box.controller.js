...
/** Stock adjustment minus: all in-hand boxes for packing (matches inventory report). */
export const getInHandBoxesByPacking = async (req, res) => {
  try {
    ...
    const rows = await findInHandBoxesByPackingNumber(pn);
...
// ─── STICKER FETCH (history) ──────────────────────────────────
/** Uses local `dailyprod` when present; otherwise live row from client `production` or IMS pack + packing_standard join (no dailyprod seed). */
export const stickerFetchBox = async (req, res) => {
  try {
    ...
    if (!rows || rows.length === 0) {
      const msg = category_id ? "No packing standard found for this category" : "No history found for this Packing No";
...
export const generateStickers = async (req, res) => {
  try {
    ...
    // 4. Data insert and status update
...
// ─── DOWNLOAD SUMMARY (packing level) ────────────────────────
