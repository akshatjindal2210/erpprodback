import {
  ...
  updateTrainingVideo,
  deleteTrainingVideo,
  approveTrainingVideoById,
} from "../models/trainingVideo.model.js";
...
export const createTrainingVideo = async (req, res) => {
  try {
    ...
    await logActivity(req, {
      ...
      entity_id: video.id,
      details: { title: video.title, approved: video.approved },
    });
    res.json({ success: true, data: video });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
...
export const updateTrainingVideoController = async (req, res) => {
  try {
    ...
    await logActivity(req, { action: "update", entity: "training_videos", entity_id: id, details: { updated_fields: Object.keys(fields) } });
    res.json({ success: true, data: video });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
...
// ─── APPROVE video (users with training_videos → authorize) ────
export const approveTrainingVideoController = async (req, res) => {
  try {
    ...
    await logActivity(req, { action: "update", entity: "training_videos", entity_id: id, details: { title: data?.title } });
...
export const deleteTrainingVideoController = async (req, res) => {
  try {
    ...
    await logActivity(req, { action: "delete", entity: "training_videos", entity_id: id });
