...
// ─── Normalize IMS rows for API / helpers ──────────────────────────
...
// ─── IMS `pack` body: `filter` with dailyprod.docdt (DDMonYYYY tokens) ─
...
/** Match ERP pack doc_no with panel `packing_number` / `dailyprod.doc_no` (30637 vs "30637.0"). */
...
export const getItemById = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      data: { ...item, category: raw.Grpname ?? item.grpname }
    });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
...
// ─── DAILY PROD (IMS pack + local sticker flag from DB) ──────────
export const getDailyProd = async (req, res) => {
  try {
    ...
    const [localGenerated, packingWithStickers] = await Promise.all([
      dbQuery(`SELECT doc_no::text AS doc_no FROM dailyprod WHERE sticker_generated = true`),
      getProductionStickerPackingDocNos(),
    ]);
...
// ─── HELPER VIEWS (permission fields on top of IMS data) ─────────
  ...
  try {
    ...
    if (onlyStickerGenerated) {
      const stickerRows = await dbQuery(
        `SELECT DISTINCT item_dcode AS itemdcode FROM dailyprod WHERE sticker_generated = true`
      );
