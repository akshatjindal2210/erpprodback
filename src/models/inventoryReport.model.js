 ...
 * All quantities come from box_table only:
...
const BOX_AGG_CTE = `
  box_agg AS (
    ...
    FROM box_table b
    LEFT JOIN location_master lm ON lm.location_id = b.location_id
...
function buildBaseCte({ filters = {}, search } = {}) {
  ...
  if (customerCodes.length) {
    ...
    boxWhere.push(`COALESCE(dp.acc_code::text, '') = ANY($${p}::text[])`);
  }
  ...
  const baseCteSql = `
    from_dailyprod AS (
      SELECT
        ...
        COALESCE(dp.acc_code::text, '—') AS customer_name
      FROM dailyprod dp
      ${dpWhereSql}
    ),
    from_boxes AS (
      SELECT
        ...
        '—'::text AS item_desc,
        MAX(dp.acc_code::text) AS customer_code,
        COALESCE(MAX(dp.acc_code::text), '—') AS customer_name
      FROM box_table b
      LEFT JOIN dailyprod dp ON ${PN_DP} = ${PN_B}
      LEFT JOIN stock_adjustment sa ON sa.adjustment_id = b.sa_id AND sa.is_deleted = false
      ${boxWhereSql}
      GROUP BY ${PN_B}
    ),
    base AS (
      SELECT * FROM from_dailyprod
      UNION
      SELECT fb.* FROM from_boxes fb
      WHERE NOT EXISTS (SELECT 1 FROM from_dailyprod fd WHERE fd.packing_number = fb.packing_number)
    )`;
  ...
  const [items, customers, packings, locations] = await Promise.all([
    ...
    dbQuery(
      `${withBase}
       ...
       FROM base rb
       INNER JOIN box_table b ON ${PN_B} = rb.packing_number AND ${SQL_IN_STORE}
       INNER JOIN location_master lm ON lm.location_id = b.location_id
