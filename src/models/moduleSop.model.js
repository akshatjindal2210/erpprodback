...
const MS_FROM = `FROM module_sops ms
  LEFT JOIN users u_cr ON ms.created_by = u_cr.id
  LEFT JOIN users u_up ON ms.updated_by = u_up.id`;
...
export const updateModuleSop = async (fields = {}, filters = {}) => {
  ...
  const [updated] = await dbQuery(
    `UPDATE module_sops SET ${setClause} WHERE ${whereClause} AND is_deleted = false RETURNING *`,
    vals
  );
  return updated ?? null;
};
...
export const deleteModuleSop = async (filters = {}, meta = {}) => {
  ...
  await dbQuery(
    `UPDATE module_sops SET is_deleted = true, deleted_at = NOW(), deleted_by = $${keys.length + 1}
