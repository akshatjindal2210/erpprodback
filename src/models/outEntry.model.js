...
// ─── JOINS ─────────────────────────
const JOINS = `
  LEFT JOIN users u_cr  ON o.created_by  = u_cr.id
  LEFT JOIN users u_upd ON o.updated_by  = u_upd.id
  LEFT JOIN users u_dl  ON o.deleted_by  = u_dl.id
  LEFT JOIN users u_ap  ON o.approved_by = u_ap.id
`;
...
export const findOutEntries = async (options = {}) => {
  ...
  // COUNT
  const [{ count }] = await dbQuery(`SELECT COUNT(*) AS count FROM out_entry o ${JOINS} ${where}`, values);
...
export const insertOutEntry = async (data) => {
  ...
  const [row] = await dbQuery(
    `INSERT INTO out_entry (fuid, remarks, created_by, scan_complete, boxes_required, boxes_scanned)
     VALUES ($1, $2, $3, $4, $5, $6)
     RETURNING *`,
...
export const updateOutEntries = async (fields = {}, filters = {}) => {
  ...
  const [row] = await dbQuery(
    `UPDATE out_entry SET ${setClause} WHERE ${whereClause} RETURNING *`,
    values
  );
  return row;
};
...
export const deleteOutEntries = async (filters = {}, meta = {}) => {
  ...
  await dbQuery(
    `UPDATE out_entry SET is_deleted = true, deleted_at = NOW(), deleted_by = $${i}
     WHERE ${conditions.join(" AND ")}`,
    values
  );
};
    ...
    json_build_object(
      ...
      'is_out_current', CASE
        WHEN $3::INTEGER IS NOT NULL AND EXISTS (
          SELECT 1 FROM out_entry_scanned_box d
...
// ─── GET FUID DETAILS FOR OUT ENTRY ────────────────
export const findFuidDetailsForOutEntry = async (fuid, forOutUid = null) => {
  ...
  // 1. Get all packing numbers for this FUID from forwarding_note_item_wise
  ...
  const itemsWithLocations = await Promise.all(packingRows.map(async (item) => {
    ...
    const [packingAreaRows, inStoreRows] = await Promise.all([
      ...
      SELECT
        ...
        ${OUT_ENTRY_BOX_JSON_AGG}
      FROM box_table b
      INNER JOIN location_master loc ON b.location_id = loc.location_id
      WHERE b.packing_number = $1
        AND ${OUT_ENTRY_BOX_AVAILABILITY_SQL}
...
export const replaceOutEntryDraftScans = async ({ out_uid, scanned_boxes = [] }) => {
  if (!out_uid) return [];
  await dbQuery(`DELETE FROM out_entry_scanned_box WHERE out_uid = $1`, [out_uid]);
  ...
  await dbQuery(
    `INSERT INTO out_entry_scanned_box (out_uid, box_no_uid) VALUES ${placeholders.join(", ")}`,
    values
  );
  return uids;
};
...
/** Boxes saved for this out entry — draft table or approved box_table links. */
export const findOutEntryLinkedBoxes = async (out_uid) => {
  if (!out_uid) return [];
  const [entry] = await dbQuery(
    `SELECT approved FROM out_entry WHERE out_uid = $1 AND is_deleted = false LIMIT 1`,
    [out_uid]
  );
  if (entry?.approved) {
    return dbQuery(
  ...
  return dbQuery(
    `SELECT d.box_no_uid::text AS box_no_uid,
            ...
            b.sa_id,
            b.sa_entry_type
     FROM out_entry_scanned_box d
     INNER JOIN box_table b ON b.box_no_uid::text = d.box_no_uid AND b.is_deleted = false
     WHERE d.out_uid = $1
...
/** Apply stock: link boxes on box_table. Call only when out entry is approved. */
...
// ─── BOX LINK MANAGEMENT FOR OUT ENTRY ─────────────────────
