...
// ─── JOINS ─────────────────────────
const JOINS = `
  LEFT JOIN users u_cr ON lm.created_by = u_cr.id
  LEFT JOIN users u_up ON lm.updated_by = u_up.id
  LEFT JOIN users u_ap ON lm.approved_by = u_ap.id
  LEFT JOIN users u_dl ON lm.deleted_by = u_dl.id
`;
...
export const findLocations = async (options = {}) => {
  ...
  // COUNT
  const countRes = await dbQuery(`SELECT COUNT(*) AS count FROM location_master lm ${JOINS} ${where}`, values);
