...
const ALLOWED_SELECT_FIELDS  = ["id", "name", "label", "sort_order", "is_active", "created_at", "updated_at", "updated_by", "updated_by_name"];
const ALLOWED_FILTER_FIELDS  = ["id", "name", "label", "sort_order", "is_active", "from_date", "to_date"];
const ALLOWED_UPDATE_FIELDS  = ["name", "label", "sort_order", "is_active", "updated_by", "updated_at"];
const ALLOWED_SORT_FIELDS    = ["id", "name", "label", "sort_order", "created_at", "is_active", "updated_at"];
...
const MODULE_LIST_JOIN = "FROM modules m LEFT JOIN users u_up ON m.updated_by = u_up.id";
...
const DEFAULT_MODULE_LIST_SELECT =
  "m.id, m.name, m.label, m.sort_order, m.is_active, m.created_at, m.updated_at, m.updated_by, u_up.name AS updated_by_name";

// ─── Find multiple modules (Users pattern) ────────────────────────
export const findModules = async (options = {}) => {
  ...
  const [{ count }] = await dbQuery(
    `SELECT COUNT(*) AS count FROM modules m ${whereClause}`,
    countValues
  );
...
export const findModule = async (filters = {}) => {
  ...
  const [module] = await dbQuery(
    `SELECT m.id, m.name, m.label, m.sort_order, m.is_active, m.created_at, m.updated_at, m.updated_by, u_up.name AS updated_by_name
     FROM modules m
     LEFT JOIN users u_up ON m.updated_by = u_up.id
     WHERE ${conditions}
     LIMIT 1`,
    values
  );
...
// ─── Insert module ────────────────────────────────────────────────
export const insertModule = async ({ name, label }) => {
  const [module] = await dbQuery(
    `INSERT INTO modules (name, label, is_active)
     VALUES ($1, $2, true)
     RETURNING id, name, label, sort_order, is_active, created_at, updated_at, updated_by`,
    [name, label]
  );
  return module;
};

// ─── Update modules dynamically (User pattern) ────────────────────
export const updateModules = async (fields = {}, filters = {}) => {
  ...
  return await dbQuery(
    `UPDATE modules SET ${setClause} 
     WHERE ${whereClause}
     RETURNING id, name, label, sort_order, is_active, updated_at, updated_by`,
    values
  );
};
