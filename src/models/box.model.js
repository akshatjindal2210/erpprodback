...
/** `sa_entry_type` must match `box_table` CHECK: 'stock_in' | 'stock_out'. */
...
const JOINS = `
  ...
  LEFT JOIN inventory_inwards ii ON b.in_uid::TEXT        = ii.in_uid::TEXT
  LEFT JOIN out_entry io        ON b.out_uid::TEXT        = io.out_uid::TEXT
  
  LEFT JOIN users u_cr    ON b.created_by  = u_cr.id
  LEFT JOIN users u_upd   ON b.updated_by  = u_upd.id
  LEFT JOIN users u_dl    ON b.deleted_by  = u_dl.id
`;
...
export const findBoxes = async (options = {}) => {
  ...
  let totalCount = 0;
  if (!skipCountQuery) {
    const countRes = await dbQuery(`SELECT COUNT(DISTINCT b.box_uid) AS count FROM box_table b ${JOINS} ${whereClause}`, values);
  ...
  const selectFields = fields.length > 0 
    ? fields.map(f => {
        ...
        // "col AS alias" must still qualify "col" — otherwise joins (e.g. dailyprod) make names like packing_number ambiguous
...
/** Resolve production acc/item when the list-view join did not attach `dailyprod` columns. */
...
/** Fallback when `dailyprod` has no linked standard: latest approved standard for the item. */
...
export const getStickerHistory = async (doc_no, category_id = null) => {
  ...
  return await dbQuery(
    ...
    LEFT JOIN ranked_standards rs
     ...
     AND rs.rn = 1
    LEFT JOIN category ct ON rs.type = ct.id
 ...
 * (`live`), not `dailyprod` — used before any local dailyprod row exists.
 */
...
/** ERP packing numbers that have production stickers in `box_table` (panel DB source of truth). */
  ...
  const rows = await dbQuery(
    `SELECT
       ...
       (
         SELECT u.name
         FROM box_table b0
         INNER JOIN users u ON u.id = b0.created_by
       ...
       (
         SELECT u.name
         FROM box_table b1
         INNER JOIN users u ON u.id = b1.updated_by
     ...
     FROM box_table b
     LEFT JOIN dailyprod dp ON dp.doc_no::text = b.packing_number::text
...
// ─── UPDATE / INSERT dailyprod AFTER STICKER GENERATION ─────────────────────────────────
...
export const updateDailyProdStickerStatus = async (doc_no, standard_id = null, snapshot = null) => {
  ...
  return dbQuery(
    `INSERT INTO dailyprod (doc_no, doc_dt, job_card_no, acc_code, item_dcode, total_qty, sticker_generated, packing_standard_id)
     VALUES (
     ...
     ON CONFLICT (doc_no) DO UPDATE SET
       ...
       acc_code = COALESCE(EXCLUDED.acc_code, dailyprod.acc_code),
       item_dcode = COALESCE(EXCLUDED.item_dcode, dailyprod.item_dcode),
       total_qty = COALESCE(EXCLUDED.total_qty, dailyprod.total_qty),
       doc_dt = COALESCE(EXCLUDED.doc_dt, dailyprod.doc_dt),
       job_card_no = COALESCE(EXCLUDED.job_card_no, dailyprod.job_card_no)
     RETURNING doc_no`,
    [
      d,
...
/** Single UPDATE — bulk “print all” without N log rows in box_download_log. */
...
const FIND_BOX_DETAILED_SELECT = `
    ...
    FROM box_table b
    LEFT JOIN dailyprod j ON b.packing_number::TEXT = j.doc_no::TEXT
    LEFT JOIN stock_adjustment sa_adj
      ON b.sa_id = sa_adj.adjustment_id AND b.sa_entry_type = 'stock_in'
    LEFT JOIN packing_standard ps ON j.packing_standard_id = ps.standard_id`;

// 2. Find Single Box Detailed (join dailyprod like findBoxes `dp` fields for suggestion resolver)
...
// ─── DOWNLOAD LOG: full download history for one box ─────────
  ...
  return await dbQuery(
    ...
    SELECT * FROM (
      ...
      SELECT
        ...
        l2.cust_at_time,
        l2.download_source
      FROM box_download_log l2
      INNER JOIN box_table b ON b.box_uid = $1
...
// ─── DOWNLOAD LOG: summary for all boxes in a packing ─────────
export const getDownloadSummaryByPacking = async (packing_number) => {
  return await dbQuery(`
    ...
    FROM box_table b
    LEFT JOIN box_download_log l ON (
      l.box_uid = b.box_uid
      OR (
...
/** One row per `box_table` sticker (legacy / box search). Pass `list_mode: "box"`. */
async function getStickerBoxManagementList(options = {}) {
  ...
  const lastLogLateral = `
    LEFT JOIN LATERAL (
      ...
      FROM (
        ...
        FROM box_download_log dl2
        LEFT JOIN users u2 ON u2.id = dl2.downloaded_by
  ...
  if (filters.from_date) {
    ...
    conditions.push(`(
      ...
      OR EXISTS (
        SELECT 1 FROM box_download_log bl
        WHERE bl.download_type = 'bulk_pack'
          AND bl.bulk_packing_number IS NOT NULL
          AND TRIM(bl.bulk_packing_number::text) = TRIM(b.packing_number::text)
...
/** One row per `box_download_log` entry (default for sticker management UI). */
async function getStickerDownloadLogList(options = {}) {
  ...
  const packingBoxCountSql = `(SELECT COUNT(*)::int FROM box_table bt2 WHERE bt2.is_deleted = false AND TRIM(COALESCE(bt2.packing_number::text, '')) = TRIM(COALESCE(l.bulk_packing_number::text, '')))`;
  ...
  const baseFrom = `
    FROM box_download_log l
    LEFT JOIN users u ON u.id = l.downloaded_by
    LEFT JOIN box_table b ON b.box_uid = l.box_uid AND b.is_deleted = false
    LEFT JOIN LATERAL (
      SELECT dp.*
      FROM dailyprod dp
...
export const insertOverrideRequest = async ({ packing_number, itemdcode, box_uids, from_customer, to_customer, remarks, requested_by, approved = false }) => {
  ...
  const [row] = await dbQuery(
    `INSERT INTO box_override_request
...
export const listOverrideRequests = async (options = {}) => {
  ...
  const joins = `
    ...
    LEFT JOIN users app_user ON app_user.id = r.approved_by
  `;

  const whereClause = `WHERE ${conditions.join(" AND ")}`;

  const countRes = await dbQuery(
    `SELECT COUNT(*) as count FROM box_override_request r ${joins} ${whereClause}`,
    values
  );
...
export const getOverrideRequestById = async (request_id) => {
  const [row] = await dbQuery(
    `SELECT * FROM box_override_request WHERE request_id = $1 LIMIT 1`,
    [request_id]
  );
  return row || null;
};
...
export const getPackingNumberFromBox = async (box_no_uid) => {
  const [row] = await dbQuery(
    `SELECT packing_number FROM box_table WHERE box_no_uid = $1 LIMIT 1`,
    [box_no_uid]
  );
