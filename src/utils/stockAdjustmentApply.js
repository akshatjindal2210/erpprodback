...
import { getBoxNoUidPrefix } from "../models/appConfig.model.js";
import {
...
import { isBoxAvailableForMinus } from "./boxInventory.js";
...
export async function applyStockAdjustmentOnApproveTx(client, { adjustment, userId }) {
  ...
  if (entryType === "minus") {
    ...
    const live = (rows || []).filter(
      (r) => !r.is_deleted && String(r.packing_number ?? "").trim() === pn
    );
