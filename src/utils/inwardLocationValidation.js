...
import { getAppConfigValue, APP_CONFIG_KEYS } from "../models/appConfig.model.js";
import { sqlBoxInHand } from "./boxInventorySql.js";

/**
 * Whether inward location–box rules are enforced.
 * `true`  → customer/item assignment on location must match the box (see boxAllowedAtLocation).
 * `false` → no checks; any box may be stored at any location.
 * 1) `app_config` row `inward_location_validation` — `"true"` / `"false"` (case-insensitive).
 * 2) If no row or empty: `INWARD_LOCATION_VALIDATION` env.
 * 3) Else `false`.
 */
export async function isInwardLocationValidationEnabled() {
  try {
    ...
    /* e.g. `app_config` not created yet */
  }
  ...
  for (const loc of locations) {
    ...
    for (const b of boxes) {
      ...
      if (!boxAllowedAtLocation(locAcc, locItem, boxAcc, boxItem)) {
        ...
        // const locOpen = locAcc == null && locItem == null;
        // console.log(locOpen && (boxAcc != null || boxItem != null));
        // if (locOpen && (boxAcc != null || boxItem != null)) {
        //   return `Box "${uid}" cannot be placed at open location "${locLabel}": use a location assigned to this box's customer/item.`;
        // }
        if (locAcc != null && boxAcc !== locAcc) {
          ...
          // return `Box "${uid}" cannot be placed at location "${locLabel}": this location is assigned to customer ${locAcc} only; other customers' stock is not allowed.`;
        }
